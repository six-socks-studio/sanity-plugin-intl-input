...
To configure the plugin you can customieze the config file called `@six-socks-studio/sanity-plugin-intl-input.json` in your studio's `config` folder. If you need specific configuration you can keep in mind, any global config can be overriden per schema.  
